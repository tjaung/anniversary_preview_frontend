...
const MessageBubble = (props) => {
  ...
  const toggleEnlarge = (index, type) => {
    if (enlarged && enlarged.index === index && enlarged.type === type) {
      setEnlarged(null); // Minimize if the same media is clicked again
    } else {
      setEnlarged({ index, type }); // Enlarge the clicked media
    }
  };
  ...
  return (
    <div
      className={`bubble-container ${props.side}`}
          ...
          <div className="images">
            {images.map((src, index) => (
              <img
                ...
                onClick={() => toggleEnlarge(index, "photo")}
                style={
                  enlarged &&
                  enlarged.index === index &&
                  enlarged.type === "photo"
                    ? {
                        // position: "fixed",
                        // top: "50%",
          ...
          <div className="images">
            {videos.map((src, index) => (
              <video
                ...
                src={src}
                controls
                onClick={() => toggleEnlarge(index, "video")}
                style={
                  enlarged &&
                  enlarged.index === index &&
                  enlarged.type === "video"
